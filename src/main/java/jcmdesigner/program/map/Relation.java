...
import jcmdesigner.graphics.gui.Arrow;
import jcmdesigner.graphics.gui.BezierCurve;
...
public class Relation extends WeightedConnection
{
	...
	public Relation(double weight, Unit start_unit, Unit end_unit)
	{
		...
		App.main_border_pane.getChildren().add(weight_text);
	...
	public void remove()
	{
		...
		App.main_border_pane.getChildren().remove(this.curve);
		App.main_border_pane.getChildren().remove(this.interaction_curve);
		App.main_border_pane.getChildren().remove(this.weight_text);
		for (Arrow arrow : this.arrows)
			App.main_border_pane.getChildren().remove(arrow);
	}
