...
public class InputOutputMenu implements Configurations
{
	private static Stage settings_stage;
	private static TextField[][] text_fields;
	...
	public void openConfigurations()
	{
		...
		InputOutputMenu.text_fields = new TextField[grid_pane.length + 1][input_vector_legth];
		...
		for (int i = 0; i < grid_pane.length; i++)
			for (int j = 0; j < input_vector_legth; j++)
			{
				...
				for (int k = 0; k <= i; k++)
				{
					InputOutputMenu.text_fields[k + i][j] = new TextField();
					InputOutputMenu.text_fields[k + i][j].setId("text_field");
					InputOutputMenu.text_fields[k + i][j].setAlignment(Pos.CENTER);
					InputOutputMenu.text_fields[k + i][j].setFocusTraversable(false);
					if (i == 0)
						InputOutputMenu.text_fields[k + i][j].setPromptText(Map.units.get(j).concept.getInput() + "");
					else
						InputOutputMenu.text_fields[k + i][j].setPromptText(Parameters.A_estimated[k][j] + "");
					InputOutputMenu.text_fields[k + i][j].setOnKeyPressed(event ->
					{
						if (event.getCode() == KeyCode.ENTER) this.buttonOnAction();
					});
					grid_pane[i].add(InputOutputMenu.text_fields[k + i][j], i + j, 1 + k);
				}
			}
	...
	{
		String[][] texts = new String[InputOutputMenu.text_fields.length][InputOutputMenu.text_fields[0].length];

		for (int i = 0; i < texts.length; i++)
			for (int j = 0; j < texts[0].length; j++)
				texts[i][j] = InputOutputMenu.text_fields[i][j].getText().toString();

		for (int i = 0; i < texts.length; i++)
			for (int j = 0; j < texts[0].length; j++)
				if (texts[i][j] != null && !texts[i][j].isEmpty()) if (i == 0)
					Map.units.get(j).concept.setInput(Double.parseDouble(texts[i][j]));
				else
					Parameters.A_estimated[i - 1][j] = Double.parseDouble(texts[i][j]);
